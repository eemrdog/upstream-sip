...
*/ -}}
{{- define "eric-pm-server.copyVCTMetadataFields" -}}
...
*/ -}}
{{- define "eric-pm-server.copyVCTAnnotations" -}}
  {{- include "eric-pm-server.copyVCTMetadataFields" (dict "metadataField" "annotations" "vctName" .vctName "customKeys" .customKeys "release" .release "name" .name) | trim -}}
{{- end -}}

{{- define "eric-pm-server.copyVCTLabels" -}}
  {{- $defaultLabels := dict "app.kubernetes.io/instance" .release.Name -}}
  {{- $mergedLabels := include "eric-pm-server.mergeLabels" (dict "location" (printf "%s.copyVCT" .name) "sources" (list $defaultLabels .customKeys)) | fromYaml -}}
  {{- include "eric-pm-server.copyVCTMetadataFields" (dict "metadataField" "labels" "vctName" .vctName "customKeys" $mergedLabels "release" .release "name" .name) | trim -}}
{{- end -}}
