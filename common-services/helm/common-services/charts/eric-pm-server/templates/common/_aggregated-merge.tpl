...
{{- define "eric-pm-server.aggregatedMerge" -}}
  {{- $merged := dict -}}
  {{- $context := .context -}}
  {{- $location := .location -}}
...
*/ -}}
{{- define "eric-pm-server.mergeAnnotations" -}}
  {{- include "eric-pm-server.aggregatedMerge" (dict "context" "annotations" "location" .location "sources" .sources) }}
{{- end -}}

{{- define "eric-pm-server.mergeLabels" -}}
  {{- include "eric-pm-server.aggregatedMerge" (dict "context" "labels" "location" .location "sources" .sources) }}
{{- end -}}
