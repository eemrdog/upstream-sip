...
*/}}
{{- define "eric-fh-snmp-alarm-provider.logshipper-global" -}}
  ...
  {{- else -}}
    {{- $globalDefaults := merge $globalDefaults (dict "registry" (dict "url" "IMAGE_URL" )) -}}
  {{- end -}}
  {{- if .Values.global }}
...
*/}}
{{- define "eric-fh-snmp-alarm-provider.logshipper-internal" -}}
...
*/}}
{{- define "eric-fh-snmp-alarm-provider.logshipper-service-fullname" -}}
...
*/}}
{{- define "eric-fh-snmp-alarm-provider.logshipper-name" -}}
...
app.kubernetes.io/name: {{ include "eric-fh-snmp-alarm-provider.name" . | quote }}
...
*/}}
{{- define "eric-fh-snmp-alarm-provider.logshipper-image" }}
{{- $productInfo := fromYaml (.Files.Get "eric-product-info.yaml") }}
...
*/}}
{{- define "eric-fh-snmp-alarm-provider.LsSeccompProfile" -}}
...
*/}}
{{- define "eric-fh-snmp-alarm-provider.logshipper-container" -}}
...
- name: "logshipper"
  ...
  image: {{ include "eric-fh-snmp-alarm-provider.logshipper-image" . }}
  args:
  ...
  securityContext:
    ...
    capabilities:
      drop:
        - "all"
    {{ include "eric-fh-snmp-alarm-provider.LsSeccompProfile" . | indent 4 }}
  env:
  - name: TZ
  ...
  - name: SERVICE_ID
    value: {{ include "eric-fh-snmp-alarm-provider.logshipper-service-fullname" . }}
  - name: CONTAINER_NAME
    value: logshipper
  livenessProbe:
    exec:
...
*/}}
{{- define "eric-fh-snmp-alarm-provider.logshipper-storage-path" }}
{{- $default := fromJson (include "eric-fh-snmp-alarm-provider.logshipper-default-value" .) }}
- name: "eric-log-shipper-storage-path"
  mountPath: {{ $default.logshipper.storagePath | quote }}
{{- end -}}
...
*/}}
{{- define "eric-fh-snmp-alarm-provider.logshipper-volume" -}}
{{- $g := fromJson (include "eric-fh-snmp-alarm-provider.logshipper-global" .) }}
{{- $default := fromJson (include "eric-fh-snmp-alarm-provider.logshipper-default-value" .) }}
- name: "eric-log-shipper-storage-path"
  emptyDir:
...
- name: "lt-client-cert"
  secret:
    secretName: "{{ include "eric-fh-snmp-alarm-provider.logshipper-service-fullname" . }}-lt-client-cert"
    optional: true
{{- end }}
{{- end -}}
{{/*
...
*/}}
{{- define "eric-fh-snmp-alarm-provider.logshipper-tls-cert-lt-client" -}}
{{- $default := fromJson (include "eric-fh-snmp-alarm-provider.logshipper-default-value" .) -}}
{{- $g := fromJson (include "eric-fh-snmp-alarm-provider.logshipper-global" .) -}}
{{- if $g.security.tls.enabled -}}
...
metadata:
  name: "{{ include "eric-fh-snmp-alarm-provider.logshipper-service-fullname" . }}-lt-client-cert"
  labels: {{- include "eric-fh-snmp-alarm-provider.logshipper-labels" . | nindent 4 }}
  annotations: {{- include "eric-fh-snmp-alarm-provider.annotations" . | nindent 4 }}
spec:
  kubernetes:
    generatedSecretName: "{{ include "eric-fh-snmp-alarm-provider.logshipper-service-fullname" . }}-lt-client-cert"
    certificateName: "clicert.pem"
    privateKeyName: "cliprivkey.pem"
  certificate:
    subject:
      cn: {{ include "eric-fh-snmp-alarm-provider.logshipper-service-fullname" . | quote }}
    issuer:
...
{{- end -}}
{{- end -}}
{{- define "eric-fh-snmp-alarm-provider.logshipper-default-value" -}}
  {{- $productInfo := fromYaml (.Files.Get "eric-product-info.yaml") -}}
  ...
  {{- else -}}
    {{- $default := merge $default (dict "imageCredentials" (dict "logshipper" (dict "repoPath" "IMAGE_PATH" ))) -}}
