...
*/ -}}
{{- define "eric-data-document-database-pg.copyVCTMetadataFields" -}}
...
*/ -}}
{{- define "eric-data-document-database-pg.copyVCTAnnotations" -}}
  {{- include "eric-data-document-database-pg.copyVCTMetadataFields" (dict "metadataField" "annotations" "vctName" .vctName "customKeys" .customKeys "release" .release "name" .name) | trim -}}
{{- end -}}

{{- define "eric-data-document-database-pg.copyVCTLabels" -}}
  {{- $commonLabels := dict "app.kubernetes.io/instance" .release.Name -}}
  {{- $mergedLabels := include "eric-data-document-database-pg.mergeLabels" (dict "location" (printf "%s.copyVCT" .name) "sources" (list $commonLabels .customKeys)) | fromYaml -}}
  {{- include "eric-data-document-database-pg.copyVCTMetadataFields" (dict "metadataField" "labels" "vctName" .vctName "customKeys" $mergedLabels "release" .release "name" .name) | trim -}}
{{- end -}}
