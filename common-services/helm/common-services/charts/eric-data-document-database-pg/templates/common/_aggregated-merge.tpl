...
{{- define "eric-data-document-database-pg.aggregatedMerge" -}}
  {{- $merged := dict -}}
  {{- $context := .context -}}
  {{- $location := .location -}}
...
*/ -}}
{{- define "eric-data-document-database-pg.mergeAnnotations" -}}
  {{- include "eric-data-document-database-pg.aggregatedMerge" (dict "context" "annotations" "location" .location "sources" .sources) | trim}}
{{- end -}}

{{- define "eric-data-document-database-pg.mergeLabels" -}}
  {{- include "eric-data-document-database-pg.aggregatedMerge" (dict "context" "labels" "location" .location "sources" .sources) | trim }}
{{- end -}}
