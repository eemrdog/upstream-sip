...
*/ -}}
{{- define "eric-ctrl-bro.copyVCTMetadataFields" -}}
...
*/ -}}
{{- define "eric-ctrl-bro.copyVCTAnnotations" -}}
  {{- include "eric-ctrl-bro.copyVCTMetadataFields" (dict "metadataField" "annotations" "vctName" .vctName "customKeys" .customKeys "release" .release "name" .name) | trim -}}
{{- end -}}

{{- define "eric-ctrl-bro.copyVCTLabels" -}}
  {{- $commonLabels := dict "app.kubernetes.io/instance" .release.Name -}}
  {{- $mergedLabels := include "eric-ctrl-bro.mergeLabels" (dict "location" (printf "%s.copyVCT" .name) "sources" (list $commonLabels .customKeys)) | fromYaml -}}
  {{- include "eric-ctrl-bro.copyVCTMetadataFields" (dict "metadataField" "labels" "vctName" .vctName "customKeys" $mergedLabels "release" .release "name" .name) | trim -}}
{{- end -}}
