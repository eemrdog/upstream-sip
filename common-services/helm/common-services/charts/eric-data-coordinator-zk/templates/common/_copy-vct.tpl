...
*/ -}}
{{- define "eric-data-coordinator-zk.copyVCTMetadataFields" -}}
...
*/ -}}
{{- define "eric-data-coordinator-zk.copyVCTAnnotations" -}}
  {{- include "eric-data-coordinator-zk.copyVCTMetadataFields" (dict "metadataField" "annotations" "vctName" .vctName "customKeys" .customKeys "release" .release "name" .name) | trim -}}
{{- end -}}

{{- define "eric-data-coordinator-zk.copyVCTLabels" -}}
  {{- $commonLabels := dict "app.kubernetes.io/instance" .release.Name -}}
  {{- $mergedLabels := include "eric-data-coordinator-zk.mergeLabels" (dict "location" (printf "%s.copyVCT" .name) "sources" (list $commonLabels .customKeys)) | fromYaml -}}
  {{- include "eric-data-coordinator-zk.copyVCTMetadataFields" (dict "metadataField" "labels" "vctName" .vctName "customKeys" $mergedLabels "release" .release "name" .name) | trim -}}
{{- end -}}
